...
3. **"Do I trust Macaw's control-flow recovery here?"** → **Macaw itself,
   ...
   result, and [`macaw-analysis.md`](macaw-analysis-results.md) for a second,
   ...
   **When Macaw is authoritative vs. when it's only a cross-check**: Macaw
   ...
   discovery path — see [`macaw-analysis.md`](macaw-analysis-results.md)'s
...
2. **Concrete (Unicorn)**: seeding `r3 = 0x26` and running from `0x888c`
   ...
   [`unicorn-backend.md`](unicorn-backend-results.md) for the exact commands.
3. **Symbolic (Crucible/What4/Z3)**: leaving `r3` symbolic and asking "is
   ...
   [`cases/performing-rigs/docs/investigations/protocol-harness-results.md`](protocol-harness-results.md).
...
3. **Solver-confirmed** — a Crucible/What4/Z3 query where the solver
   ...
   [`docs/harness/execution-model.md`](symbolic-harness-results.md) about
...
> [`cases/performing-rigs/docs/investigations/trigger-input.md`](trigger-input.md).
...
producing) — see [`macaw-analysis.md`](macaw-analysis-results.md)'s "Cortex-M
...
load-bearing for the IR Crucible executes, but its discovered function
boundaries, block partitioning, and terminator classification are not
ground truth — see [`macaw-analysis.md`](macaw-analysis-results.md) for a second,
...
concrete — see
[`cases/performing-rigs/docs/investigations/protocol-pipeline.md`](protocol-pipeline.md)
for the full trace.
...
[`cases/performing-rigs/config/svd/ATSAMD51J19A.svd`](../../config/svd/ATSAMD51J19A.svd), with a
small standalone resolver,
[`cases/performing-rigs/config/svd/resolve_mmio.py`](../../config/svd/resolve_mmio.py), that
...
[`cases/performing-rigs/docs/investigations/boot-and-hardware-bringup.md`](boot-and-hardware-bringup.md)
...
([`cases/performing-rigs/research/provenance/function_classification.csv`](../../research/provenance/function_classification.csv))
...
([`cases/performing-rigs/research/provenance/ghidra_labels.tsv`](../../research/provenance/ghidra_labels.tsv)).
...
See [`docs/toolchain.md`](../../../../docs/toolchain.md) for exact install/build steps for
