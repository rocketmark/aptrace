...
- Database: `cases/performing-rigs/research/runs/census/census.sqlite3` (override with `--db`).
...
- Dynamic-coverage capture files (intermediate, one per scenario):
  `cases/performing-rigs/research/runs/census/dynamic/<scenario>.json`.
...
"
tools/unicorn/.venv/bin/python3 tools/census/aptrace_census.py ingest-dynamic cases/performing-rigs/research/runs/census/dynamic
```
...
`reference_library.py` from `cases/performing-rigs/docs/investigations/boot-and-hardware-
...
- **`AUTOPILOT_RECIPE`** (reference `autopilot868`) — packages,
  VERBATIM, every disclosed assumption from `cases/performing-rigs/docs/investigations/
...
what `cases/performing-rigs/docs/investigations/boot-and-hardware-bringup.md`'s own toolchain
...
|---|---|---|
| `adafruit/ArduinoCore-samd` | `1.7.11` | PRIMARY target — named directly by embedded build-path strings in all four firmware images (`cases/performing-rigs/docs/firmware/firmware-layout.md`) |
...
- **No physical-connector identity.** `pins` never claims "PA08 = Motor
  ...
  attempt it. See `cases/performing-rigs/docs/hardware/hardware-reference.md` if such curated
  mappings exist.
...
# of downloaded/compiled artifacts under cases/performing-rigs/research/runs/census/reference_corpus/,
