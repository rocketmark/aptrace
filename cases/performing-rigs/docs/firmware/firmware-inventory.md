...
Source: `~/github/aptrace/cases/performing-rigs/research/firmware/vendor-package/` (untouched originals) and
`cases/performing-rigs/research/firmware/originals/` (working copies + hashes; the `.bin` files
...
- **`autopilot`** — the motion-control unit. This is APTrace's current and
  only target; see [`cases/performing-rigs/status.md`](../../status.md)'s
...
- **`mando`** — the RF remote control. Not yet analyzed by APTrace beyond
  ...
  `cases/performing-rigs/research/autopilot_static_inventory/` pass, which reverse-engineered
...
- Flashing mechanism: `cases/performing-rigs/research/firmware/vendor-package/update_firmware.bat` invokes `bossac`
  ...
  [`cases/performing-rigs/docs/firmware/firmware-layout.md`](firmware-layout.md) for what this
