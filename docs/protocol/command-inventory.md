...
| `B0\|` / `B1\|` / `B2\|` | Remote -> AutoPilot | B-family mode/state command. **`B0\|`'s Remote-side sender confirmed** (`manual-mode-wire-provenance.md`): a shared settings/row-action widget (`FUN_0000d988`/`FUN_0000db34`), sent 4x on a specific confirmed row selection — not a Manual Mode jog command itself | inline state change |
| `TR0\|` / `TR1\|` | Remote -> AutoPilot | Boolean family (false/true). **`TR0\|`'s Remote-side sender confirmed** (`manual-mode-wire-provenance.md`): same shared settings/row-action widget, sent 3x on a different row selection | inline state change |
...
| `LL1\|` / `LL2\|` | Remote -> AutoPilot | First/second limit workflow — **execution-confirmed**: `LL1` clears two globals + a validity flag; `LL2` orders them and sets the flag only if they differ. Neither touches live position, target/config, or `0x20001b14`; no GPIO/MMIO dependency; no other firmware code writes either global with a real value — see [`ll-limit-workflow.md`](../investigations/ll-limit-workflow.md). **Both `LL1`'s and `LL2`'s Remote-side senders are now CONFIRMED, and they are the same function**: `FUN_0000de3c`, the same screen that hosts the real live-jog loop (see the `0xF0`/`0xE0` row below). `LL1` is sent 3x as an entry preamble AND 3x again after the first jog phase's click, gated on a real position-query success (`I9\|`/`I1\|`, next row); `LL2` is sent 3x after the second jog phase's click, same gate. Found by an exhaustive whole-image raw-byte scan (exactly one `"LL2\|"` literal anywhere in the image) cross-checked by an independent Ghidra xref scan (exactly one reference, at `0xe114`, inside `FUN_0000de3c`) — no other candidate sender exists. The position query's parsed numeric result is confirmed **not** forwarded into either `LL` send (both remain bare, argument-less frames at the sender, `FUN_000058a8`) — see [`ll2-set-limits-provenance.md`](../investigations/ll2-set-limits-provenance.md) | — |
| `I9\|` / `I1\|` | Remote -> AutoPilot | **Sender now CONFIRMED** (previously listed below as "short forms... used by a separate Remote routine," sender unknown): `FUN_0000b834`, a real position-query helper — sends one of these two (selected by a mode byte, `*0x200018e7`), waits up to ~1000-3000 ticks (6 retries) for a response, and parses a signed decimal number from it, matching `I`'s own documented `<signed-number>,` (event 15) response shape. Its two call sites are the two "click to confirm a limit position" moments inside `FUN_0000de3c`'s Set-Limits sequence (row above) — success gates whether `LL1`/`LL2` are (re)sent; the parsed value itself is stored at a fixed Remote-RAM scratch slot (`0x200027f0`) and, by exhaustive xref, is never read again by anything. See [`ll2-set-limits-provenance.md`](../investigations/ll2-set-limits-provenance.md) | `<signed-number>,` (event 15, same as `I`) |
| `H\|` / `J\|` | Remote -> AutoPilot | Toggle a global flag (opposite directions) | — |
| `0xF0 <payload>` / `0xE0 <payload>` (2nd variant) | **Remote -> AutoPilot — direction now CONFIRMED** | **The real Manual Mode / live-jog wire mechanism**, not a discrete ASCII command. Checked by the AutoPilot's `ascii_dispatcher` (`FUN_00008258`) *before* any ASCII command. Grammar (`0xF0`, fully decoded; `0xE0` structurally similar, not byte-mapped): `<0xF0/0xE0><len><0xFF x4>[<channel nibble><sign+23-bit value>]*<'\|'><seq>`, sent by `FUN_0000be94` (called from the live-jog loop `FUN_0000de3c`, Part 3 of the evidence doc) once per UI tick for as long as the jog wheel is not clicked — real captured frames: idle `b'\xf0\x07\xff\xff\xff\xff\|'`, one record `b'\xf0\x0b\xff\xff\xff\xff\x00LK@\|'` (channel 0, value 5,000,000). AutoPilot's receiver routes each record's channel/value into `FUN_00005274`/`FUN_00004d18` — **the same entry point the `I<channel><mode>\|` command uses** — cross-confirmed via the shared `0x20000180[channel]` array. The per-channel value's exact semantic source (accumulated jog delta vs. live/target position) and the `0xE0` variant's precise field widths remain **UNKNOWN**. See [`manual-mode-wire-provenance.md`](../investigations/manual-mode-wire-provenance.md) | — |
...
| `T<0 or 1023>,<1 or 0>,\|` | AutoPilot -> Remote | **New this pass, found while tracing the 3.5mm trigger-input reported bug** — a real, previously undocumented outbound frame. Sent from inside the `MC4`-gated motor-phase/ramp state machine (`phase_ramp_state_machine__CUSTOM`), rate-limited to once per ~500 firmware ticks, reporting a live `digitalRead()` of the trigger circuit's PB05 pin: `"T1023,0,\|"` when PB05 reads high that cycle, `"T0,1,\|"` when low. Sent through the AutoPilot's already-proven TX wrapper (`0x8c10`). A level-sampled poll with no debounce/history between checks — see [`trigger-input-concrete-path.md`](../investigations/trigger-input-concrete-path.md) | — |
